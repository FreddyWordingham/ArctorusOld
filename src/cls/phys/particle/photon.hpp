/**
 *  @author Freddy Wordingham
 *  @email  fjmw201@exeter.ac.uk
 *
 *  @date   07/02/2018.
 */



//  == GUARD ==
#ifndef ARCTORUS_SRC_CLS_PHYS_PARTICLE_PHOTON_HPP
#define ARCTORUS_SRC_CLS_PHYS_PARTICLE_PHOTON_HPP



//  == BASE CLASS ==
#include "cls/phys/particle.hpp"



//  == INCLUDES ==
//  -- System --
#include <vector>

//  -- Classes --
#include "cls/graphical/point/photon.hpp"
#include "cls/phys/material.hpp"



//  == NAMESPACE ==
namespace arc
{
    namespace phys
    {
        namespace particle
        {



            //  == CLASS ==
            /**
             *  Photon packet particle class.
             */
            class Photon : public Particle
            {
                //  == FIELDS ==
              private:
                //  -- Properties --
                const double m_wavelength;  //! Packet wavelength.

                //  -- Optical --
                double m_ref_index;     //! Current refractive index.
                double m_albedo;        //! Current albedo.
                double m_interaction;   //! Current interaction coefficient.
                double m_anisotropy;    //! Current anisotropy value.

                //  -- Path --
                std::vector<graphical::point::Photon> m_path; //! Path data of the photon.


                //  == INSTANTIATION ==
              public:
                //  -- Constructors --
                Photon(const math::Vec<3>& t_pos, const math::Vec<3>& t_dir, double t_time, double t_weight,
                       double t_wavelength, double t_ref_index, double t_albedo, double t_interaction, double t_anisotropy);
                Photon(const math::Vec<3>& t_pos, const math::Vec<3>& t_dir, double t_time, double t_weight,
                       double t_wavelength, const phys::Material& t_mat);


                //  == METHODS ==
              public:
                //  -- Getters --
                double get_wavelength() const { return (m_wavelength); }
                double get_ref_index() const { return (m_ref_index); }
                double get_albedo() const { return (m_albedo); }
                double get_interaction() const { return (m_interaction); }
                double gem_anisotropy() const { return (m_anisotropy); }
                const std::vector<graphical::point::Photon>& get_path() const { return (m_path); }

                //  -- Setters --
                void set_dir(const math::Vec<3>& t_dir)
                {
                    assert(t_dir.is_normalised());

                    n_dir = t_dir;
                }

                //  -- Simulation --
                void move(double t_dist);
                void scatter();

              private:
                //  -- Data --
                void record_path();
            };



        } // namespace particle
    } // namespace phys
} // namespace arc



//  == GUARD END ==
#endif // ARCTORUS_SRC_CLS_PHYS_PARTICLE_PHOTON_HPP
